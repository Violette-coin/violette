...
// violette: if pwallet != NULL it will attempt to create coinstake
std::unique_ptr <CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript &scriptPubKeyIn, bool fMineWitnessTx, CWallet *pwallet, bool *pfPoSCancel) {
    ...
    // violette: if coinstake available add coinstake tx
    ...
    if (pwallet)  // attemp to find a coinstake
    {
        *pfPoSCancel = true;
        ...
        if (*pfPoSCancel)
            return nullptr; // violette: there is no point to continue if we failed to create coinstake
    }
...
//        LOCK(cs_main);
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("VioletteMiner: generated block is stale");
    }
...
void PoSMiner(CWallet *pwallet) {
    ...
    RenameThread("violette-stake-minter");
    ...
    try {
        ...
        while (true) {
            ...
            // violette: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake()) {
...
// violette: stake minter
void MintStake(boost::thread_group &threadGroup) {
    // violette: mint proof-of-stake blocks in the background
    if (!vpwallets.empty())
...
void static VioletteMiner(const CChainParams &chainparams, void *parg) {
    LogPrintf("VioletteMiner started\n");
    RenameThread("violette-miner");
    CWallet *pwallet = (CWallet *) parg;
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("Error in VioletteMiner: Keypool ran out, please call keypoolrefill before restarting the mining hread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running VioletteMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//                        LOCK2(cs_main, pwallet->cs_wallet);
                        if (!SignBlock(*pblock, *pwallet)) {
                            LogPrintf("VioletteMiner(): failed to sign PoS block");
                            continue;
                        }
        ...
        LogPrintf("VioletteMiner terminated!\n");
        throw;
    }
    catch (const std::runtime_error &e) {
        LogPrintf("VioletteMiner runtime error: %s\n", e.what());
        return;
    }
}

void GenerateViolette(bool fGenerate, int nThreads, const CChainParams &chainparams) {
    ...
    if (!vpwallets.empty()) {
        for (int i = 0; i < nThreads; i++)
            minerThreads->create_thread(boost::bind(&VioletteMiner, boost::cref(chainparams), vpwallets[0]));
    }
}
